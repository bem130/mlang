        ...
        TypedAstNode::FnDef {
            ...
            for (param_name, param_type) in params {
                compiler.wat_buffer.push_str(&format!(
                    " (param ${} {})",
                    param_name,
                    compiler.type_to_wat(param_type)
                ));
            }
            ...
            if *return_type != DataType::Unit {
                compiler
                    .wat_buffer
                    .push_str(&format!(" (result {})", compiler.type_to_wat(return_type)));
            }
        ...
        for (unique_name, data_type) in locals {
            ...
            if data_type != DataType::Unit {
                compiler.wat_buffer.push_str(&format!(
                    "    (local ${} {})\n",
                    unique_name,
                    compiler.type_to_wat(&data_type)
                ));
            }
        }
    }
