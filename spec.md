...
* 不変な変数に対して `name = expr` 形式の代入文を記述するとコンパイルエラーになる。
...
`while` 構文は `while <condition> { ... }` という形を取り、条件式が `true` の間ブロックを繰り返し実行します。パーサーは専用の
`RawAstNode::While` を生成し、条件と本体の両方を既存の式/ブロック解析ロジックに委ねます。

意味解析では以下を保証します。

* 条件式は必ず `bool` 型でなければならない。`i32` など他の型を返す式を条件に記述するとコンパイルエラーになる。
* 本体は暗黙に新しいスコープを持つブロックとして扱われ、内部で宣言された変数はループ外へ漏れない。
* 本体の評価結果は常に `()` である必要があり、値を返す式を末尾に書いた場合はエラーになる。

Typed AST では `TypedExprKind::While` に変換され、Wasm コード生成段階で `block` / `loop` / `br_if` 命令を用いた標準的なループ構造に
低レベル化されます。結果として `while` 式自体も `()` 型となり、ブロックの最後に置いた場合でも返り値を持たない制御フローとして扱われます。
